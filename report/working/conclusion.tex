...
\begin{enumerate}
  \item \textbf{Prove Conjecture~\ref{conj:algorithm-succeeds} for $n \ge 3$}
    (\Cref{sec:construct-repair-remains}). This is now the top-ranked direction
    by a wide margin, and it has a proved base case: completeness holds at
    $n = 2$ for every $m$ (Theorem~\ref{thm:n2-complete}), constructively and
    in $O(m\tau)$ time, by the $h$-window technique --- a discrete
    intermediate-value argument on the aggregate $u = \tilde{v}_1 +
    \tilde{v}_2$ along swap paths, closed off by extremal endpoint arguments.
    Soundness, certification, termination and polynomial complexity are
    theorems for every $n$
    (Theorems~\ref{thm:algorithm-soundness} and~\ref{thm:algorithm-complexity}).
    What remains is the $n \ge 3$ analogue of the window argument: a
    one-dimensional aggregate whose per-move variation is smaller than the
    forbidden gap it must cross, controlling $\binom{n}{2}$ pairwise
    comparisons at once. The $14$ known local optima at $n = 3$ bound what any
    such proof must accommodate, and Remark~\ref{rem:proof-driven-refinement}
    records the move-set refinement (pairwise exchanges) the proof already
    forced on the implementation.
