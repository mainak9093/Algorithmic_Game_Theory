...
We have not settled Conjecture~\ref{conj:main}. Three strategies that a reader
would reasonably try are ruled out: item-by-item insertion, which is the strategy
that works in~\cite{BKNS22} (Theorem~\ref{thm:obstruction}); restriction to
utilitarian-optimal allocations, together with \emph{every} refinement of that
set (Proposition~\ref{prop:msw-false}, Corollary~\ref{cor:no-refinement}); and
...
\begin{enumerate}
  ...
  \item \textbf{Induct on agents rather than items.} Every approach here inducts
    on items; \cite[\S4.3.1]{LMS26} adds agents one at a time, maintaining an
    equality graph rather than a subsidy scalar. For chores a newcomer starts
    with the cheapest possible bundle and is the natural sink, so the
    orientation problem of \Cref{sec:approach1} does not arise, and the
    equality graph is precisely the extra information the previous item says a
    recipient rule needs. Untried, and currently the most promising untouched
    route.
