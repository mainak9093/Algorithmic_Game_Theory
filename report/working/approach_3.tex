...
Lemma~\ref{lem:free-peel-safe} or Lemma~\ref{lem:paid-peel}, no terminal is
reachable from the root on \emph{any} of $191$ test instances.
\end{proposition}

The reason is visible in the calculus rather than the data. At the root every
$\workprofile_i = \items$, so every arc weight is $0$ and $\pathw{} \equiv 0$.
Hence $p = 0$ is the potential, \emph{every} arc is tight, and
Proposition~\ref{prop:tight-arc} degenerates to ``$\mu_k = 0$ for all $k$'', i.e.\
to freeness; while Lemma~\ref{lem:paid-peel} needs $p_x = 1$ and so cannot fire at
all. Both lemmas are blind near the root.
...
The object that is not blind at the root is the whole set of admissible
potentials. Identify $p \in \set{0,1}^n$ with the paid set
