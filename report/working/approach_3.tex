...
\begin{proposition}[Out-arcs are free; only in-arcs can fail]
\label{prop:inarcs-only}
Let $S$ be any subset of $\agents$. After $\mathrm{peel}(x,j)$, the constraints on
arcs \emph{out of} $x$ hold for $S$ whenever they held before, and the
constraints on arcs not incident to $x$ are unchanged. Consequently
\begin{align*}
  S \in \paidsets(\workprofile')
  \iff\ & S \text{ satisfies the non-$x$ constraints of } \workprofile, \\
  \text{and}\ \ & \edgew{\workprofile}(i,x) + \mu_i \le \lambda_S(i,x)
    \quad \text{for all } i \ne x .
\end{align*}
\end{proposition}

\begin{proof}
Lemma~\ref{lem:peel} leaves arcs not incident to $x$ untouched and replaces
$\edgew{}(x,k)$ by $\edgew{}(x,k) - \mu_x \le \edgew{}(x,k)$, so any bound of the
form $\edgew{}(x,k) \le \lambda_S(x,k)$ survives. The remaining constraints are
those on arcs into $x$, which become $\edgew{}(i,x) + \mu_i \le \lambda_S(i,x)$.
\end{proof}
