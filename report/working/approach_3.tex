...
\paragraph{Half the rule is now a theorem.} Lemma~\ref{lem:peel} makes the free
case immediate.
...
This is the peel-side counterpart of Theorem~\ref{thm:freeinsert}, and it was
checked on $674{,}062$ free peels with $0$ violations. Combined with
Lemma~\ref{lem:balance-free} --- balance-preservation costs nothing --- a free
peel onto any chore $j$ with $\abs{S_j} \ge 2$ and any owner other than
$f(j)$ is unconditionally safe. Over the reachable restricted states,
$58.7\%$ have such a peel available, so the rule is proved correct on the
majority of its steps.
