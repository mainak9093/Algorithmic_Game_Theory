...
%%
%%  Source: CRI.md (2026-08-09) and update_47/.  Every lemma here is proved and
...
%%
%%  Scripts: update_47/cri_anchor.py    (the lemmas)
%%           update_47/cri_sweep.py     (0 bad roots; the refutations)
%%           update_47/cri_stuck.py     (the contingency tables)
%%           update_47/cri_where.py     (stuckness sits at |R| <= 2)
%%           update_47/cri_lookahead.py (no dead state at |R| >= 3)
%%           update_47/cri_witnesses.py (the minimal witness for each refutation)
%% ==========================================================================
...
from all seven generators of \texttt{update\_44/counterexample\_hunt.py} rather
...
(\texttt{update\_47/cri\_sweep.py}).
\end{observation}
...
(\texttt{update\_47/cri\_lookahead.py}).
\end{observation}
...
(\texttt{update\_48/depth\_stress.py}):
\begin{center}\small
\begin{tabular}{@{}lrrr@{}}
\toprule
family & $m = 6$ & $m = 7$ & $m = 8$ \\
