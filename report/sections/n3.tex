%% ==========================================================================
%%  The Conjecture for Three Agents
%%
%%  Promoted from working/approach_12.tex.  Report form: no step-by-step
%%  scaffolding, no motivating example, no status paragraph, no verification
%%  appendix.  Every definition it needs beyond Section 2 is stated here.
%%
%%  REFERENCING: theorem-like environments share one counter in this preamble,
...
We prove Conjecture~\ref{conj:main} for $n = 3$. No assumption is made on the
cost functions beyond Definition~\ref{def:dichcost}, and none on the number of
chores.
...
We work in cost form throughout and write
$\cost_i(g \mid S) := \cost_i(S \cup \set{g}) - \cost_i(S) \in \set{0,1}$ for a
marginal. The proof completes an envy-free \emph{partial} allocation supplied
by the following external result; recall from \Cref{sec:prelim} that
Definition~\ref{def:ef} and Definition~\ref{def:efsolution} apply verbatim to
partial allocations.
...
chores. If $R = \emptyset$ the allocation is already complete and
Theorem~\ref{thm:n3-main} holds with $\subsidy = 0$. The cases $\abs R = 1$ and
$\abs R = 2$ are settled in \Cref{sec:n3-one} and \Cref{sec:n3-two}
respectively, using the quantity introduced next.
...
Theorem~\ref{thm:n3-twyz}, not merely to that theorem's statement, and the
distinction is not cosmetic. The hypotheses of Lemma~\ref{lem:n3-stopping} do
...
three agents and three bundles, which the Hungarian method performs in constant
time after the nine oracle calls determining $\cost_a(Y_j)$. Hence the whole
