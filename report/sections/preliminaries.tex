...
Proposition~\ref{prop:zero-coordinate} justifies the word ``hence'' in
Conjecture~\ref{conj:main}: the bound on the \emph{total} is not an independent
requirement but a consequence of the per-agent one. The conjecture is therefore a
single-clause statement,
\[
  \exists\, \alloc \ :\quad \max_{i \in \agents} \pathw{\alloc}(i) \;\le\; 1,
\]
and it is this form that the rest of the report works with. Two cautions are
worth recording alongside it. First, the bound cannot be improved: the family of
