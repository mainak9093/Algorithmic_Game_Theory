...
We will also use the standard relaxation of envy-freeness. Note that
...
since it is the form the results below establish.
...
$\cost_i(\bundle i) \le \cost_i(\bundle j)$, or there is a chore
$e \in \bundle i$ with
\[
  \cost_i\bigl(\bundle i \setminus \set e\bigr) \;\le\; \cost_i(\bundle j).
\]
\end{definition}
...
The following two results underpin much of the literature on subsidies for envy-freeness. Both are stated
...
that an envy-freeable allocation can always be obtained from an arbitrary
one: fix the bundles and reassign them according to a maximum-weight perfect
matching between agents and bundles. The central question is therefore not the existence of an envy-freeable allocation, but the amount of subsidy guaranteed by Theorem~\ref{thm:hs-minsubsidy}.

Specialising in our model gives the following, which we use throughout.
