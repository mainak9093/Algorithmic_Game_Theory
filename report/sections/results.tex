...
Conjecture~\ref{conj:main} is open. Five restrictions of it are not, and they are
collected here because together they delimit what a general proof still has to
reach. The first four are cases of the valuation class or of the instance; the
fifth is a criterion, and it is the only one that says anything about three
agents in general.
...
\begin{remark}[What it leaves open]
\label{main:rem-spread-gap}
Theorem~\ref{main:sigma3} is conditional on a family with $\Sigma \le 3$
existing. Whether one always does is open, and is the single statement to which
Conjecture~\ref{conj:main} at $n = 3$ has been reduced. Two facts bound the
question. If no uniformly balanced family exists then, since each agent can
individually be brought to spread at most $1$ by the greedy rule of
Theorem~\ref{main:identical}, a family with $\Sigma \le 3$ must give some agent
spread exactly $0$. And for binary additive costs such an agent exists only when
$3$ divides $\abs{D_i}$, so the question turns on a divisibility condition on the
underlying sets. The full development is deferred.
\end{remark}
