...
% --------------------------------------------------------------------------
\subsection{The completion is EF1}
\label{sec:m-ef1}

\begin{corollary}
\label{cor:ef1}
The complete allocation $\alloc$ of \eqref{eq:m-alloc} is EF1 in the sense of
Definition~\ref{def:ef1}. More precisely, every non-recipient
$i \in \agents \setminus T$ envies nobody in $\alloc$, and for every recipient
$t_k \in T$ the removal of the single residual chore $e_k$ from her own bundle
eliminates all of her envy at once.
\end{corollary}

\begin{proof}
By \eqref{eq:m-alloc} we have $X_j \subseteq A_j$ for every $j \in \agents$.

Let $i \in \agents \setminus T$, so that $A_i = X_i$. For every $j \in \agents$,
\[
  \cost_i(A_i) = \cost_i(X_i) \le \cost_i(X_j) \le \cost_i(A_j),
\]
the first inequality by envy-freeness \eqref{eq:m-pef} of $X$ and the second by
monotonicity (Observation~\ref{obs:m-gap}) applied to $X_j \subseteq A_j$. So
$i$ envies nobody, and Definition~\ref{def:ef1} is met at $i$ with
$X = \emptyset$.

Let $i = t_k \in T$, so that $A_i = X_i \cup \set{e_k}$ and hence
$A_i \setminus \set{e_k} = X_i$. For every $j \in \agents$,
\[
  \cost_i\bigl(A_i \setminus \set{e_k}\bigr) = \cost_i(X_i)
  \le \cost_i(X_j) \le \cost_i(A_j),
\]
by the same two steps. The bundles of $\alloc$ are pairwise disjoint, so
$e_k \notin A_j$ for $j \ne i$ and therefore
$\cost_i(A_j) = \cost_i(A_j \setminus \set{e_k})$; Definition~\ref{def:ef1} is
met at $i$ with $X = \set{e_k}$.

Since one and the same chore works against every $j$ simultaneously, $\alloc$
is EF1.
\end{proof}
...
that $(\alloc,\subsidy)$ is an envy-free solution. The allocation $\alloc$ is
moreover EF1. Both $\alloc$ and $\subsidy$ are computable in polynomial time
given value-oracle access.
\end{theorem}
...
being envy-free it is EF1, taking $X = \emptyset$ in Definition~\ref{def:ef1}.
...
$\sum_i \subsidy_i \le n-1$. Corollary~\ref{cor:ef1} gives that $\alloc$ is EF1.
