...
when $\items \setminus A_j = A_i$, that is, only when $n = 2$\ifworking; see
Remark~\ref{rem:complement}\fi. For $n \ge 3$ there is no such reduction, and that is
...
two-sided form\ifworking\ of \Cref{def:ef1}\fi, which removes one item from
...
Conjecture~\ref{conj:main} remains open. What stands in the way is recorded
below: three techniques that settle the neighbouring corners of the square above,
each of which fails here, and for each an explicit instance exhibiting the
failure. Several special cases are settled; work towards the general statement is
in progress and is not reported here.
\ifworking
...
asks for. That money is genuinely needed here is not obvious a priori but is
...
dichotomous  & \cite{BKNS22}  & \textbf{Conjecture~\ref{conj:main}} \\
\bottomrule
\end{tabular}
\end{center}

\noindent
each of the three settled corners giving one dollar per agent and $n-1$ in
total. The present question is the fourth. Note also that~\cite{LMS26} does not
displace~\cite{KMSTY24} as the baseline of \Cref{sec:intro}: for valuations that
are dichotomous but not additive, $n-1$ per agent from~\cite{KMSTY24} remains the
best published bound.

We are not aware of any treatment of envy-freeness with subsidy for chores under
dichotomous costs. This is the outcome of a literature search rather than a
proof of novelty --- \cite{LMS26} appeared in July 2026, which is a reminder of
how quickly the surrounding corners are being filled in --- and it should be
repeated before the paper is circulated.
