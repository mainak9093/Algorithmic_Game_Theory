...
\todo{write this paragraph once Approach~1 is settled. It should mirror the
``Our Results'' paragraph of~\cite{BKNS22}: one sentence stating the theorem,
one sentence on the algorithmic guarantee in the value-oracle model, one
sentence on tightness pointing at Example~\ref{ex:lowerbound}, and one sentence saying
which structural assumptions are \emph{not} needed. If the outcome is a
separation instead, state the counterexample and the largest subsidy bound that
does survive.}
