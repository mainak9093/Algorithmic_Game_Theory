...
The third route turns the chore problem into a \emph{goods} problem, to which
the theorem of~\cite{BKNS22} applies verbatim, and enforces what is left over by
pricing. The translation is exact, and stating it is what makes the obstruction
in \Cref{sec:main-pricing} intelligible.
...
the $\set{0,1}$ target transfers in both directions without loss.
...
Applying~\cite{BKNS22} to $\hat{I}$ is legal by
Theorem~\ref{main:replica}(i) and returns some $B$ with
$\hat{\subsidy} \in \set{0,1}^n$ and total at most $n-1$. But it is free to place
two copies of one type in one bundle, in which case that chore is absent from at
least two bundles, has at least two owners, and the image under $\Phi^{-1}$ is
not a partition. By Corollary~\ref{main:coverage} coverage is the sole residual
difficulty.

The natural repair is to make duplicates unattractive --- by staging the
allocation so that each chore's copies are dealt out together, with the later
