...
asymmetry is subtler than the table suggests\ifworking, and it is what
Theorem~\ref{thm:obstruction} exploits\fi. Under goods, the harm done by an
...
--- is not controlled by any condition on $\subsidy_x$.
\ifworking
That gap is not a defect of the analysis; \Cref{sec:approach1} shows it is
realised.
\else
Whether that gap can actually be realised is the question an incremental
algorithm has to answer.
\fi
...
rather than building the allocation up one chore at a time.
\ifworking
That is the one place where the incremental template killed in
\Cref{sec:approach1} is not obviously an obstacle. In the proof of
...
Complementation relates the chore and goods models only for
$n = 2$\ifworking\ (Remark~\ref{rem:complement})\fi. There is a second transform,
available for every $n$, which does not solve the problem but says precisely how
far it is from being a corollary of~\cite{BKNS22}.
...
as a black box.
\ifworking
Nor can its algorithm simply be re-run with a balancing tie-break, because
Theorem~\ref{thm:obstruction} rules out the entire item-by-item template on
which it is built.
\fi
The transform also shows the chore problem is at least as expressive as the
goods problem, so no easier.
\end{remark}
\fi
