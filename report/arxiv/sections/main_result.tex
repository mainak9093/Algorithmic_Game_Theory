...
$R(X) := \items \setminus \bigcup_{i \in \agents} X_i$ for its \emph{residue}. Moreover, as $\cost_i$s are integer valued, for all $S,T\subseteq M$, \(\cost_i(S) < \cost_i(T) \implies \cost_i(S) \le \cost_i(T) - 1 .
\)
...
\begin{enumerate}
 ...
 \item[(R3)] Otherwise, if there exists a tail strongly connected
      component $S$ of $G$ such that
      \[
          \abs{R(X)}\geq \abs S,
      \] select such a component and assign distinct residual chores to the agents of $S$, one chore per agent. If no such tail strongly
      connected component exists, halt.
\end{enumerate}
Consequently, if the algorithm terminates with $R(X)\ne\emptyset$, then neither (R1) nor (R2) is applicable at the terminal state, and
\[
    \abs{R(X)}<\abs S
\]
for every tail strongly connected component $S$ of the terminal equality graph.
...
\[
    R:=R(X), \qquad r:=\abs R.
\]
Unless stated otherwise, let $G=(\agents,E)$ denote the equality graph defined in~\eqref{eq:m-graph} at this terminal state. When $r\geq 1$, fix an arbitrary tail strongly connected component $S$ of $G$. Such a component exists by \Cref{def:scc}, and the terminal condition of \textnormal{(R3)} ensures that
\(
    r<\abs S.
\)
...
\begin{proof}
(a) Since $r\geq 1$, the algorithm terminates with a nonempty residue. Hence neither \textnormal{(R1)} nor \textnormal{(R2)} is applicable at the terminal state. Moreover, by the terminal condition of
\textnormal{(R3)},
\[
    r<\abs C
\]
for every tail strongly connected component $C$ of $G$. Since $S$ is a tail strongly connected component, this gives $r<\abs S$, proving
(a).
...
\State Let $S$ be an arbitrary tail strongly connected component of the terminal equality graph $G$. \Comment{$r<|S|$ by the terminal condition of (R3)}
