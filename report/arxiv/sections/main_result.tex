...
\[
    R:=R(X), \qquad r:=\abs R.
\]
Unless stated otherwise, let $G=(\agents,E)$ denote the equality graph defined in~\eqref{eq:m-graph} at this terminal state. When $r\geq 1$, fix an arbitrary tail strongly connected component $S$ of $G$. Such a component exists by \Cref{obs:tailscc}, and the terminal condition of \textnormal{(R3)} ensures that
\(
    r<\abs S.
\)
