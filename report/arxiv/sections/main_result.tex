...
\[
    R:=R(X), \qquad r:=\abs R.
\]
Unless stated otherwise, let $G=(\agents,E)$ denote the equality graph defined in~\eqref{eq:m-graph} at this terminal state. When $r\geq 1$,
fix an arbitrary tail strongly connected component $S$ of $G$, whose
existence follows from \Cref{lem:tail-scc-exists}. By the terminal
condition of \textnormal{(R3)},
\(
    r<\abs S.
\)
...
Algorithm~3 of~\cite{tao2025existence}; the completion and subsidy
...
Lemma~\ref{lem:m-terminal}(i), $\cost_i(A_i) = \cost_i(X_i) + \cost_i(e \mid X_i)
...
\begin{itemize}
  ...
  \item[(1b)] $j \in P \setminus T$. Then $\subsidy_j = 1$ and $A_j = X_j$. By
    ...
    Lemma~\ref{lem:m-terminal}(iii) gives $\cost_i(X_i) < \cost_i(X_j)$, hence
    $\cost_i(X_j) \ge \cost_i(X_i) + 1$ by integrality of the cost functions. Hence
...
\begin{itemize}
  ...
  \item[(3b)] $j \in T$. Then $\subsidy_j = 1$ and $A_j = X_j \cup \set e$ for
    ...
    $\cost_i(X_j) \ge \cost_i(X_i) + 1$ by integrality of the cost functions, and
  ...
  \item[(3c)] $j \in P \setminus T$. Then $\subsidy_j = 1$ and $A_j = X_j$; we
    ...
    by Lemma~\ref{lem:m-Pstructure}, and Lemma~\ref{lem:m-terminal}(iii) gives
    $\cost_i(X_i) < \cost_i(X_j)$. In either case
    integrality of the cost functions yields $\cost_i(X_j) \ge \cost_i(X_i) + 1$, so
    $\cost_i(A_j) - \subsidy_j = \cost_i(X_j) - 1 \ge \cost_i(X_i)$.
\end{itemize}
